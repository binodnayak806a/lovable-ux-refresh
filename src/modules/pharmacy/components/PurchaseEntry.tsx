...
export default function PurchaseEntry() {
  ...
  const searchMedicines = async (query: string) => {
    if (query.length < 2) { setMedOptions([]); return; }
    const { data } = await supabase
      .from('medications')
      .select('id, generic_name, brand_name, strength')
      .eq('hospital_id', hospitalId)
      .eq('is_active', true)
      .or(`generic_name.ilike.%${query}%,brand_name.ilike.%${query}%`)
      .limit(10);
    setMedOptions((data ?? []).map((m: Record<string, string>) => ({
      id: m.id,
  ...
  const addMedicine = (med: MedOption) => {
    setItems(prev => [...prev, {
    ...
    try {
      ...
      for (const item of items) {
        const { data: existing } = await supabase
          ...
          .eq('hospital_id', hospitalId)
          .eq('medication_name', item.medicine_name)
          .eq('batch_number', item.batch)
          .maybeSingle();

        if (existing) {
          const ex = existing as { id: string; quantity_in_stock: number };
          await supabase
            .from('pharmacy_inventory')
            .update({
              quantity_in_stock: ex.quantity_in_stock + item.quantity,
                ...
                {items.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="text-center py-8 text-gray-400">
                      Search or add medicines to create a purchase entry
                    </td>
                  </tr>
                ) : (
                  items.map((item) => (
                    <tr key={item.id} className="border-b hover:bg-gray-50/50">
                      ...
                      <td className="px-3 py-2">
                        <input
                          ...
                          placeholder="Batch"
                          className="w-full h-7 px-2 rounded border border-gray-200 text-sm font-mono"
                        />
                      </td>
                      <td className="px-3 py-2">
                        <input
