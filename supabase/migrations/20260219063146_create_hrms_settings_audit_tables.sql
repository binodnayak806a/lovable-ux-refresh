/*
  # HRMS, Settings, and Audit Log Tables

  ## New Tables

  ### staff
  - Full employee record: personal info, designation, department, employment details
  - Status tracking: active, on_leave, resigned, terminated
  - Links to hospital via hospital_id

  ### attendance
  - Daily attendance per staff member
  - Tracks check_in, check_out times
  - Statuses: present, absent, half_day, leave
  - Unique constraint on (staff_id, date)

  ### leave_requests
  - Leave applications with type, date range, reason
  - Approval workflow: pending → approved/rejected
  - approved_by references profiles table

  ### system_settings
  - Key-value store for hospital configuration
  - Categorized by setting_type (general, billing, clinical, security, notifications)
  - Unique per (hospital_id, setting_key)

  ## Security
  - RLS enabled on all tables
  - Staff/attendance/leaves: authenticated users in same hospital can read; admins can write
  - System settings: admins can read and write
  - Audit logs: already exist in DB; policies allow admin read
*/
