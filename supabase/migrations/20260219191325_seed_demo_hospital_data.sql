/*
  # Seed Demo Hospital, Wards, Beds, Service Items, and Ambulances

  1. Data Seeded
    - Demo hospital record (City General Hospital)
    - 15 departments
    - 8 wards with 98 beds
    - 48 service items across all categories
    - 3 demo ambulances
    - 20 investigations
*/
...
BEGIN
  ...
  IF w1 IS NULL THEN SELECT id INTO w1 FROM wards WHERE hospital_id = '11111111-1111-1111-1111-111111111111' AND name = 'General Ward A'; END IF;
  ...
  IF w2 IS NULL THEN SELECT id INTO w2 FROM wards WHERE hospital_id = '11111111-1111-1111-1111-111111111111' AND name = 'General Ward B'; END IF;
  ...
  IF w3 IS NULL THEN SELECT id INTO w3 FROM wards WHERE hospital_id = '11111111-1111-1111-1111-111111111111' AND name = 'Private Ward'; END IF;
  ...
  IF w4 IS NULL THEN SELECT id INTO w4 FROM wards WHERE hospital_id = '11111111-1111-1111-1111-111111111111' AND name = 'Semi-Private Ward'; END IF;

  INSERT INTO wards (hospital_id, name, ward_type, total_beds, available_beds, floor, block, daily_rate)
  VALUES ('11111111-1111-1111-1111-111111111111', 'ICU', 'icu', 10, 10, 3, 'A', 8000)
  ON CONFLICT DO NOTHING RETURNING id INTO w5;
  IF w5 IS NULL THEN SELECT id INTO w5 FROM wards WHERE hospital_id = '11111111-1111-1111-1111-111111111111' AND name = 'ICU'; END IF;

  INSERT INTO wards (hospital_id, name, ward_type, total_beds, available_beds, floor, block, daily_rate)
  VALUES ('11111111-1111-1111-1111-111111111111', 'HDU', 'hdu', 8, 8, 3, 'B', 5000)
  ON CONFLICT DO NOTHING RETURNING id INTO w6;
  IF w6 IS NULL THEN SELECT id INTO w6 FROM wards WHERE hospital_id = '11111111-1111-1111-1111-111111111111' AND name = 'HDU'; END IF;
  ...
  IF w7 IS NULL THEN SELECT id INTO w7 FROM wards WHERE hospital_id = '11111111-1111-1111-1111-111111111111' AND name = 'Maternity Ward'; END IF;
  ...
  IF w8 IS NULL THEN SELECT id INTO w8 FROM wards WHERE hospital_id = '11111111-1111-1111-1111-111111111111' AND name = 'Pediatric Ward'; END IF;

  FOR i IN 1..20 LOOP
    INSERT INTO beds (hospital_id, ward_id, bed_number, bed_type, status, daily_rate) VALUES ('11111111-1111-1111-1111-111111111111', w1, 'GWA-' || LPAD(i::text, 2, '0'), 'general', 'available', 500) ON CONFLICT DO NOTHING;
  END LOOP;
  FOR i IN 1..20 LOOP
    INSERT INTO beds (hospital_id, ward_id, bed_number, bed_type, status, daily_rate) VALUES ('11111111-1111-1111-1111-111111111111', w2, 'GWB-' || LPAD(i::text, 2, '0'), 'general', 'available', 500) ON CONFLICT DO NOTHING;
  END LOOP;
  FOR i IN 1..10 LOOP
    INSERT INTO beds (hospital_id, ward_id, bed_number, bed_type, status, daily_rate) VALUES ('11111111-1111-1111-1111-111111111111', w3, 'PVT-' || LPAD(i::text, 2, '0'), 'general', 'available', 2500) ON CONFLICT DO NOTHING;
  END LOOP;
  FOR i IN 1..10 LOOP
    INSERT INTO beds (hospital_id, ward_id, bed_number, bed_type, status, daily_rate) VALUES ('11111111-1111-1111-1111-111111111111', w4, 'SPV-' || LPAD(i::text, 2, '0'), 'general', 'available', 1500) ON CONFLICT DO NOTHING;
  END LOOP;
  FOR i IN 1..10 LOOP
    INSERT INTO beds (hospital_id, ward_id, bed_number, bed_type, status, daily_rate) VALUES ('11111111-1111-1111-1111-111111111111', w5, 'ICU-' || LPAD(i::text, 2, '0'), 'icu', 'available', 8000) ON CONFLICT DO NOTHING;
  END LOOP;
  FOR i IN 1..8 LOOP
    INSERT INTO beds (hospital_id, ward_id, bed_number, bed_type, status, daily_rate) VALUES ('11111111-1111-1111-1111-111111111111', w6, 'HDU-' || LPAD(i::text, 2, '0'), 'oxygen', 'available', 5000) ON CONFLICT DO NOTHING;
  END LOOP;
  FOR i IN 1..10 LOOP
    INSERT INTO beds (hospital_id, ward_id, bed_number, bed_type, status, daily_rate) VALUES ('11111111-1111-1111-1111-111111111111', w7, 'MAT-' || LPAD(i::text, 2, '0'), 'general', 'available', 1200) ON CONFLICT DO NOTHING;
  END LOOP;
  FOR i IN 1..10 LOOP
    INSERT INTO beds (hospital_id, ward_id, bed_number, bed_type, status, daily_rate) VALUES ('11111111-1111-1111-1111-111111111111', w8, 'PED-' || LPAD(i::text, 2, '0'), 'general', 'available', 800) ON CONFLICT DO NOTHING;
  END LOOP;
END $$;
...
-- Ambulances
INSERT INTO ambulances (hospital_id, vehicle_number, vehicle_type, make_model, driver_name, driver_phone, current_status) VALUES
